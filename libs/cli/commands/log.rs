...
pub async fn short_output(tasks: &[Task]) -> eyre::Result<()> {
    ...
    for task in tasks.iter() {
        let start_naive = DateTime::from_timestamp(task.start as i64, 0)
            .ok_or(eyre::eyre!("Failed to create NaiveDateTime"))?;
        let start_time = Utc.from_utc_datetime(&start_naive.naive_utc());

        //let end_time = if let Some(end) = task.end {
        let (humanized, duration) = if let Some(end) = task.end {
            let end_naive = DateTime::from_timestamp(end as i64, 0)
                .ok_or_else(|| eyre::eyre!("Failed to create NaiveDateTime"))?;
            let end_time = Utc.from_utc_datetime(&end_naive.naive_utc());
...
pub async fn verbose_output(tasks: &[Task]) -> eyre::Result<()> {
    ...
    for task in tasks.iter() {
        ...
        let start_naive = DateTime::from_timestamp(task.start as i64, 0)
            .ok_or(eyre::eyre!("Failed to create NaiveDateTime"))?;
        let start_time = Utc
            .from_utc_datetime(&start_naive.naive_utc())
            .format("%Y-%m-%d %H:%M:%S")
            .to_string();
        ...
        let end_time = if let Some(end) = task.end {
            let end_naive = DateTime::from_timestamp(end as i64, 0)
                .ok_or_else(|| eyre::eyre!("Failed to create NaiveDateTime"))?;
            Utc.from_utc_datetime(&end_naive.naive_utc())
                .format("%Y-%m-%d %H:%M:%S")
                .to_string()
        } else {
            "ONGOING".to_string()
