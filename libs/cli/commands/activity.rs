...
struct Session<'a> {
    ...
    app_durations: HashMap<String, u64>,
}

impl<'a> Session<'a> {
    fn new(first_activity: &'a ActivityDto) -> Self {
        let mut app_durations = HashMap::new();
        let duration_ms = first_activity
            .last_active
            .saturating_sub(first_activity.start);
        app_durations.insert(first_activity.app_name.clone(), duration_ms);
    ...
    fn add_activity(&mut self, activity: &'a ActivityDto) {
        let duration_ms = activity.last_active.saturating_sub(activity.start);
        *self
            .app_durations
...
/// Groups activities into sessions based on time gaps and cumulative application focus.
fn group_activities_into_sessions(activities: &[ActivityDto]) -> Vec<Session<'_>> {
    ...
    for i in 1..activities.len() {
        ...
        if gap_ms > SESSION_GAP_THRESHOLD_MS {
            ...
            let duration_ms = current_act.last_active.saturating_sub(current_act.start);

            let cumulative_secondary_app_time = current_session
                .app_durations
                .get(&current_act.app_name)
                .cloned()
                .unwrap_or(0);

            if cumulative_secondary_app_time + duration_ms > FOCUS_SHIFT_THRESHOLD_MS {
                create_new_session = true;
            }
        }
...
fn print_activities_grouped(activities: &[ActivityDto], detailed_view: bool) {
    ...
    for (i, session) in sessions.iter().enumerate() {
        ...
        if Some(date) != last_date {
            ...
            println!("{}", format!("◆ {}", date.format("%Y-%m-%d")).blue().bold());
            println!("{}", "│".dimmed());
            last_date = Some(date);
