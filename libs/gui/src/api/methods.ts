...
const method = function <Ret, T extends unknown[]>(name: string, m?: (...args: T) => Record<string, unknown>): (...args: T) => Promise<Ret> {
  (window as any).__METHOD = { ...((window as any).__METHOD ?? {}), [name]: m };
  return (...args: T) => invoke(name, m?.(...args) ?? {});
}

export const listLastTasks = method<Task[], [number]>('list_last_tasks', (count: number) => ({ count }));

export const stopCurrentTask = method('stop_current_task');

export const deleteTaskByUlid = method('delete_task_by_ulid', (task_id: string) => ({ ulid: task_id }));
...
export const editTask = method('edit_task', (task_id: string, task_update: EditTaskUpdateInput) => ({ ulid: task_id, data: task_update }));
...
export const saveNewConfig = method('save_new_config', (config: SaveNewConfigInput) => ({ config: config }));
