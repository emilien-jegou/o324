...
export function useFloating(
  internalPlacement: Placement = 'top',
  trigger = 'hover',
  noArrow = false,
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    function onClick(event: MouseEvent) {
      if (
        [floatingRef.value].some(
          (element) =>
            element === event.target || (element && element.contains(event.target as HTMLElement)),
        )
      )
        return;
    ...
    const cleanupFn =
      triggerRef.value && floatingRef.value
        ? autoUpdate(triggerRef.value, floatingRef.value, updatePosition)
        : () => {};
