import { $, useSignal, component$, useResource$, Resource } from '@builder.io/qwik';
import { z } from 'zod';
...
//import { TimePeriodInput } from '~/ui/common/time-period-input';
import { GrowingMultiSelect } from './custom-multi-select';
import type { Signal } from '@builder.io/qwik';
//import type { TimePeriodValue } from '~/ui/common/time-period-input';
...
const editTaskSchema = z.object({
  ...
  period: z.object({ start: z.number(), end: z.number().optional() }),
});
...
export const EditTaskModal = component$((props: EditTaskModalProps) => {
  ...
  const timePeriodValue = useSignal({
    start: props.task.start,
    end: props.task.end ?? props.task.start,
  });
  ...
  const form = useForm<ReachOutFormLogicData>('edit-task-form', {
    defaultValue: {
      ...
      period: { start: props.task.start, end: props.task.end ?? undefined },
    },
  ...
  return (
    <Modal bind:show={props['bind:show']} contentClass="w-[800px] h-auto">
      <h2 class="text-base font-medium mb-6 text-ellipsis overflow-hidden">Edit task</h2>
      <Resource
        value={data}
          ...
          <form class="flex gap-1 flex-col" preventdefault:submit onSubmit$={form.formSubmit$}>
            ...
            <div class="flex">
              {/*<TimePeriodInput
                name="period"
                class="w-full"
                bind:value={timePeriodValue}
                onChange$={(d: TimePeriodValue) => form.setFormValue$('period', d)}
              />*/}
            </div>
